...
const RegisterPage = () => {
   ...
   const [errorMessageIndustri, setErrorMessageIndustri] = useState("");
   ...
   const handleRegis = (event) => {
      ...
      if (!industry) {
         setErrorMessageIndustri(<p className="text-red-600 mt-2 ml-2">Silahkan isi jenis instansi anda!</p>);
         return;
      }
      if (!password) {
      ...
      // aksi register dari redux     
      dispatch(register(full_name, email, phone_number, company_name, industry, password, navigate))
   }
            ...
            <div className="px-4 py-2 lg:flex">
               ...
               <div className="border-2 rounded-2xl py-8 px-14 shadow-lg border-slate-300 bg-blue-100 lg:bg-transparent lg:w-1/2 lg:border-none lg:shadow-none">
                  ...
                  <form action="submit" onSubmit={handleRegis} className="flex flex-col gap-4">
                     ...
                     {/* Industry */}
                     <div className="relative">
                        <label htmlFor="industri" className="w-full font-medium flex flex-col-reverse gap-2">
                           <input
                              id="industri"
                              type="text"
                              name="industri"
                              className="border-2 py-2 px-4 rounded-3xl shadow-md lg:px-6 lg:py-3 lg:border-[1px] placeholder:font-light placeholder:text-sm"
                              placeholder="Jenis Perusahaan anda"
                              value={industry}
                              onChange={(event) => {
                                 setindustry(event.target.value);
                                 setErrorMessageIndustri("");
                              }}
                           />
                           Jenis Industri
                        </label>
                        {errorMessageIndustri && <p className="text-color-warn text-xs mb-2">{errorMessageIndustri}</p>}
                        <LiaIndustrySolid className="absolute top-10 right-4 w-6 h-6 text-slate-400 lg:top-11" />
                     </div>
                     {/* password */}
                     ...
                     <div className="">
                        <button type="submit" className="border-2 bg-blue-700 py-3 px-4 font-medium text-white rounded-3xl w-full mt-4 text-sm">Sign Up</button>
                     </div>
                  </form>
               </div>
               {/* card foto mode web */}
